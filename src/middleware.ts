...
const PUBLIC_PATHS       = new Set([
  "/", "/auth/signin", "/auth/signout", "/auth/error",
  "/api/auth", "/api/health",
]);
...
export default auth(async (req: NextRequest & { auth?: { user?: { id?: string; role?: string } } }) => {
  ...
  if (pathname.startsWith("/api/v1")) {
    ...
    if (botCheck.isBot && botCheck.confidence > 0.8) {
      return NextResponse.json(
        { success: false, error: { code: "FORBIDDEN", message: "Automated requests not allowed" } },
        { status: 403 },
      );
    }
  }
  ...
  if (isProtected || isAdminRoute) {
    const session = (req as NextRequest & { auth?: { user?: { id?: string; role?: string } } }).auth;
